...
export function PPDocs({ go, onBack, read, markRead }) {
  const investorDocs = PRIVATE.docs.investor;
  ...
  const DocRow = ({ d }) => (
    <div onClick={() => { markRead(d.id); go(`doc_view:${d.id}`); }} style={{
      ...
      <span style={{ color: d.action === "download" ? C.teal : C.teal, fontSize: 18 }}>›</span>
    </div>
  );
  ...
  return (
    <Shell onBack={onBack}>
      ...
      <Card style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 12 }}>
        <div style={{ position: "relative", width: 64, height: 64 }}>
          <svg width="64" height="64" viewBox="0 0 64 64">
            <circle cx="32" cy="32" r="28" fill="none" stroke={C.cardHi} strokeWidth="6" />
            <circle cx="32" cy="32" r="28" fill="none" stroke={C.teal} strokeWidth="6"
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex",
            ...
            fontWeight: 700, fontSize: 15, color: C.teal }}>{pct}%</div>
        </div>
        <div>
          ...
          <div style={{ fontSize: 14, color: C.text, marginTop: 4 }}>
            Review each document with your advisors.
          </div>
        </div>
      </Card>
      ...
      <div className="portal-two-col-wide" style={{ marginTop: 8 }}>
        <div>
          ...
          <Card style={{ padding: "2px 18px" }}>
            {PRIVATE.docs.company.map((d) => <DocRow key={d.id} d={d} />)}
          </Card>
        </div>
        ...
        <div>
          ...
          <Card style={{ padding: "2px 18px" }}>
            {PRIVATE.docs.investor.map((d) => <DocRow key={d.id} d={d} />)}
          </Card>
        </div>
      </div>
...
// =============================================================================
// PP2.5 — Q&A + AI Investor Assistant (Private · 506(c) framing)
// =============================================================================
